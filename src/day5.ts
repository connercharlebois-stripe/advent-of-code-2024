...
type PageOrderRule = {
    page: number,
    mustComeBeforePage: number
}
type Update = {
    pages: number[],
    valid?: boolean,
    brokenRules?: PageOrderRule[]
}

const parseRules = (input: string): PageOrderRule[] => {
...
const verifyUpdate = (update: Update, rules: PageOrderRule[]) : PageOrderRule[] => {
    const brokenRules :PageOrderRule[] = []
...
const getMiddlePageFromUpdate = (update: Update) : number => {
    const length = update.pages.length /2;
    return update.pages[Math.floor(length)];
}

const stupidlyFixBrokenRules = (update: Update, brokenRules: PageOrderRule[], allRules: PageOrderRule[]) : void => {
...
const process = (input: string, valid: boolean) => {
    let sum;
