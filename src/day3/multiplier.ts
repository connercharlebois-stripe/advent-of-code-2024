...
export const process = (input: string, useToggles: boolean) => {
    const re = new RegExp(/mul\((\d+),(\d+)\)/g)
    const re_toggles = new RegExp(/do\(\)|don't\(\)/g);
    let match, matches: RegExpExecArray[] = [], toggles_match, toggles: RegExpExecArray[] = []
...
export default () => {
    ...
    console.log(process(sample2, true));
}