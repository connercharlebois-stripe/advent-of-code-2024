...
const getCoordsInDirection = (row: number, col: number, direction: EDirection): { row: number, col: number } => {
    switch (direction) {
        case EDirection.N:
            return {
                row: row - 1,
...
const getDirFromDxDy = (dx: number, dy: number): EDirection => {
    if (dx === -1 && dy === -1) {
...
const isAdjacentToLetter = (data: string[][], row: number, col: number, letter: string, direction: EDirection): { row: number, col: number, dir: EDirection }[] | null => {
...
export const crossMasSearch = (input: string) => {
    const data: string[][] = []
    const rows = input.split("\n");
    rows.forEach(r => {
        data.push(r.split(""));
    ...
    for (let r = 0; r < data.length; r++) {
        for (let c = 0; c < data[r].length; c++) {
            if (data[r][c] === "A") {
                let misses = 0, hits: {row:Number, col:Number, dir:EDirection}[] =[];
                count_a++;
                console.log("A", { c, r });
